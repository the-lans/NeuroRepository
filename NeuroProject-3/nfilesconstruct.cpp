...
{
    if(fexamples.is_open() && isFileExamples)
    {
        fexamples.close();
    }
    if(fneuronet.is_open() && isFileNeuronet)
    {
        fneuronet.close();
    }
    if(ftrainbp.is_open() && isFileTrainBP)
    {
        ftrainbp.close();
    }
}
...
{
    if(tmp_fexamples.is_open())
    {
        tmp_fexamples.close();
    }
    if(tmp_fneuronet.is_open())
    {
        tmp_fneuronet.close();
    }
    if(tmp_ftrainbp.is_open())
    {
        tmp_ftrainbp.close();
    }
}
