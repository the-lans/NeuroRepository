...
public:
    NExample<NType>* original; //Оригинальный пример
...
public:
    void copyExample(NExample<NType>* obj); //Копирование примера
    void copyTmpExample(NExample<NType>* obj);
    void deinit(); //Деинициализация
    void init(int lenIn, int lenOut, NType value); //Инициализация массивов значением
    bool isOriginal(); //Оригинальный пример или временный
...
{
    this->original = (obj.isOriginal() ? this : obj.original);
...
template <typename NType>
void NExample<NType>::copyExample(NExample<NType>* obj)
{
    this->original = (obj->isOriginal() ? this : obj->original);
    this->input = obj->input;
    this->output = obj->output;
    this->outrun = obj->outrun;
    this->outpostrun = obj->outpostrun;
    this->outputNumClass = obj->outputNumClass;
    this->energy = obj->getEnergy();
    this->typeSet = obj->getTypeSet();
    this->usage = obj->getUsage();
}
...
template <typename NType>
void NExample<NType>::copyTmpExample(NExample<NType>* obj)
{
    this->original = obj;
    this->outrun = obj->outrun;
    this->energy = obj->getEnergy();
...
template <typename NType>
bool NExample<NType>::isOriginal()
{
...
void NExample<NType>::saveECSV(DataECSV& dt, string& parent)
{
    ...
    dt.addGroup(parent, "");
    if(this->isOriginal()) {to_array_string(str_vec, this->input); dt.addElement(parent, "input", str_vec, typeid(NType).name());}
    if(this->isOriginal()) {to_array_string(str_vec, this->output); dt.addElement(parent, "output", str_vec, typeid(NType).name());}
    ...
    if(this->isOriginal()) {to_array_string(str_vec, this->outputNumClass); dt.addElement(parent, "outputNumClass", str_vec, typeid(int).name());}
