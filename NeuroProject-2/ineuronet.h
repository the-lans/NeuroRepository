...
public:
    ...
    NType (*targetAver)(NType&, int&);
    NType (*targetMax)(NType&);
...
public:
    ...
    virtual void runExample(NExample<NType>* exm); //Выполнение примера
    virtual void runExampleDropout(NExample<NType>* exm);
    ...
    NType funcEnergy(NExample<NType>* exm); //Целевая функция
    NArray<NType>& derivEnergy(NExample<NType>* exm); //Производная целевой функции
    void funcActivLay(NArray<NType>& out); //Функция активации слоя
    NArray<NType>* derivActivLay(NExample<NType>* exm); //Производная функции активации слоя
...
NArray<NType>* INeuroNet<NType>::run(NArray<NType>* X)
{
    ...
    if(examples != nullptr) {examples->outrun = input;}
...
template <typename NType>
void INeuroNet<NType>::runExample(NExample<NType>* exm)
{
    //Функционирование нейросети
    this->run(&(exm->input));

    //Копирование выходного вектора
    exm->outrun.memCopy(this->examples->outrun->getData(), shiftout, this->examples->outrun->getLength());
...
template <typename NType>
void INeuroNet<NType>::runExampleDropout(NExample<NType>* exm)
{
    //Функционирование нейросети
    this->run_dropout(&(exm->input));
...
void INeuroNet<NType>::runExamples(NSetType st)
{
    ...
    if(st == NSetType::NSetTrain)
    {
        ...
        endExm = this->examples->getTestset();
    }
    else if(st == NSetType::NSetTest)
    {
        begExm = this->examples->getTestset();
        endExm = this->examples->getValidset();
    }
    else if(st == NSetType::NSetValidate)
    {
        begExm = this->examples->getValidset();
        endExm = this->examples->getEndset();
    }

    for(int pos = begExm; pos < endExm; pos++)
    {
        exm = this->examples->get(pos);
        this->runExample(exm);
    }
...
void INeuroNet<NType>::runEnergy(NSetType st)
{
    ...
    {
        this->examples->setEnergyAver(this->targetAver(enrgSum, lenExm));
    }
    ...
    {
        this->examples->setEnergyMax(this->targetMax(enrgMax));
    }
}
...
{
    NType* pOutput = exm->output.getData() + shiftout;
    NType* pOutrun = exm->outrun.getData() + shiftout;
...
template <typename NType>
NArray<NType>* INeuroNet<NType>::derivActivLay(NExample<NType>* exm)
{
    if(derivActivationLay != nullptr)
    {
        derivActivationLay(derivLay, exm->output, exm->outrun);
        return &derivLay;
    }
    return nullptr;
}
...
bool INeuroNet<NType>::check()
{
    ...
    INLayer<NType>** pLay = this->lay.getData();
    NExample<NType>** pExm = this->examples->getData();
    ...
    if(lenlay > 0)
    {
        ...
        if(pLay[0]->weigth.getLenRow() != this->examples->getLenIn()) {erCheck = NError::NErrorStruct;}
        if(pLay[lenlay-1]->weigth.getLenColumn() != this->examples->getLenOut()) {erCheck = NError::NErrorStruct;}
        if(erCheck != NError::NErrorNone) {return false;}

        //Проверка примеров
        if(this->examples->getTestset() < this->examples->getBeginset()
                || this->examples->getValidset() < this->examples->getTestset()
                || this->examples->getEndset() < this->examples->getValidset()
                || this->examples->getEndset() > this->examples->getLength())
        {
            erCheck = NError::NErrorCrossExm;
        }
        if(erCheck != NError::NErrorNone) {return false;}

        for(int indExm = 0; indExm < this->examples->getLength() && erCheck == NError::NErrorNone; indExm++)
        {
           if(pExm[indExm]->input.empty()) {erCheck = NError::NErrorExmEmpty;}
           if(pExm[indExm]->output.empty()) {erCheck = NError::NErrorExmEmpty;}
           if(pExm[indExm]->outrun.empty()) {erCheck = NError::NErrorExmEmpty;}
        }
        if(erCheck != NError::NErrorNone) {return false;}
